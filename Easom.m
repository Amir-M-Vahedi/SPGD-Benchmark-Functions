...
tic
while Iter<=Iter_max & abs(O_gsa+1)>=tel
...
f2=plot3(x_g_History(:,1),x_g_History(:,2),x_g_History(:,3),'b-',"LineWidth",1, 'DisplayName', 'GD');

plot3(xb_gsa(1),xb_gsa(2),O_gsa,'r.',"MarkerSize",15)
f3=plot3(x_gsa_History(:,1),x_gsa_History(:,2),x_gsa_History(:,3),'r.-',"LineWidth",1, 'DisplayName', 'SPGD');
...
f8=plot3(x_pg_History(:,1),x_pg_History(:,2),x_pg_History(:,3),'m-.',"LineWidth",1, 'DisplayName', 'PGD');

plot3(Fmincon_b(1),Fmincon_b(2),Fmincon_b(3),'k.',"MarkerSize",15)
f9=plot3(optimhistory.x(:,1),optimhistory.x(:,2),optimhistory.fval,'k-',"LineWidth",1, 'DisplayName', 'Fmincon');
...
plot(x_g_History(:,3),'ob',"LineWidth",1,'MarkerSize',10)
% plot(x_g_History(:,3),'b',"LineWidth",1)
hold on
plot(x_pg_History(:,3),'m-.',"LineWidth",3)
plot(history_sa.fval,'--','Color','#EDB120',"LineWidth",4.5)
plot(optimhistory.fval,'k+',"LineWidth",1,'MarkerSize',10)
...
z=[exp((- (x - pi)^2 - (y - pi)^2))*cos(y)*sin(x) + exp((- (x - pi)^2 - (y - pi)^2))*cos(x)*cos(y)*(2*x - 2*pi),...
   exp((- (x - pi)^2 - (y - pi)^2))*cos(x)*sin(y) + exp((- (x - pi)^2 - (y - pi)^2))*cos(x)*cos(y)*(2*y - 2*pi)];
end
...
if nargout > 1 % gradient required
    g=[exp((- (x(1) - pi)^2 - (x(2) - pi)^2))*cos(x(2))*sin(x(1)) + exp((- (x(1) - pi)^2 - (x(2) - pi)^2))*cos(x(1))*cos(x(2))*(2*x(1) - 2*pi),...
   exp((- (x(1) - pi)^2 - (x(2) - pi)^2))*cos(x(1))*sin(x(2)) + exp((- (x(1) - pi)^2 - (x(2) - pi)^2))*cos(x(1))*cos(x(2))*(2*x(2) - 2*pi)];
end
