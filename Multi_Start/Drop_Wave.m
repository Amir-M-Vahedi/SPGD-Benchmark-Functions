...
tel = 1e-7;                % Termination tolerance
...
function grad = Object_prime(x, y)
    ...
    common_cos = cos(12 * r) + 1;
    common_sin = sin(12 * r);

    dfdx = (x * common_cos) / denom^2 + (12 * x * common_sin) / (r * denom);
    dfdy = (y * common_cos) / denom^2 + (12 * y * common_sin) / (r * denom);
...
function [f, g] = ObjectMinco(x)
    ...
    if nargout > 1
        common_cos = cos(12 * r) + 1;
        common_sin = sin(12 * r);
        dfdx = (x(1) * common_cos) / denom^2 + (12 * x(1) * common_sin) / (r * denom);
        dfdy = (x(2) * common_cos) / denom^2 + (12 * x(2) * common_sin) / (r * denom);
        g = [dfdx, dfdy];
    end
end
