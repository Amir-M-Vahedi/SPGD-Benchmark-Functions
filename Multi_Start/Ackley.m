...
function [xbest, fval, T, fcount] = runBayesOpt(x0, Iter_max)
    ...
    results = bayesopt(@bayesObjectiveFcn, vars, ...
        'MaxObjectiveEvaluations', Iter_max/5, ...
...
function grad = Object_prime(x, y)
    grad = [pi*exp(0.5*(cos(2*pi*x)+cos(2*pi*y)))*sin(2*pi*x) + ...
           (2*x*exp(-sqrt(x^2/2 + y^2/2)/5))/sqrt(x^2/2 + y^2/2), ...
           pi*exp(0.5*(cos(2*pi*x)+cos(2*pi*y)))*sin(2*pi*y) + ...
           (2*y*exp(-sqrt(x^2/2 + y^2/2)/5))/sqrt(x^2/2 + y^2/2)];
end
...
function [f, g] = ObjectMinco(x)
    ...
    if nargout > 1
        g = [pi*exp(0.5*(cos(2*pi*x(1))+cos(2*pi*x(2))))*sin(2*pi*x(1)) + ...
             (2*x(1)*exp(-sqrt(x(1)^2/2 + x(2)^2/2)/5))/sqrt(x(1)^2/2 + x(2)^2/2), ...
             pi*exp(0.5*(cos(2*pi*x(1))+cos(2*pi*x(2))))*sin(2*pi*x(2)) + ...
             (2*x(2)*exp(-sqrt(x(1)^2/2 + x(2)^2/2)/5))/sqrt(x(1)^2/2 + x(2)^2/2)];
    end
end
