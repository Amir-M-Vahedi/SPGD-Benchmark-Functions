...
Iter_max = 2.5e3;            % Max iterations for all methods
lr = 0.01;                % Learning rate
tel = 1e-6;                % Termination tolerance
rng(1);                    % For reproducibility
initial_points = 100 * (2 * rand(nStarts, 2) - 1);
...
function [xbest, fval, T, fcount] = runSPGD(x0, lr, tel, Iter_max)
    ...
    for iter = 1:Iter_max
        ...
        if mod(iter, Iter_p) == 0
            ...
            test_list = max(min(test_list, ub), lb);  % clamp to [lb, ub]
...
function [xbest, fval, T, fcount] = runBayesOpt(x0, Iter_max)
    vars = [optimizableVariable('x1', [-100, 100]), ...
            optimizableVariable('x2', [-100, 100])];
...
function grad = Object_prime(x, y)
    exp_term = exp(-((x - pi)^2 + (y - pi)^2));
    grad_x = exp_term * cos(y) * sin(x) + exp_term * cos(x) * cos(y) * (2*x - 2*pi);
    grad_y = exp_term * cos(x) * sin(y) + exp_term * cos(x) * cos(y) * (2*y - 2*pi);
    grad = [grad_x, grad_y];
end
...
function [f, g] = ObjectMinco(x)
    exp_term = exp(-((x(1) - pi)^2 + (x(2) - pi)^2));
    f = -cos(x(1)) * cos(x(2)) * exp_term;

    if nargout > 1
        grad_x = exp_term * cos(x(2)) * sin(x(1)) + exp_term * cos(x(1)) * cos(x(2)) * (2*x(1) - 2*pi);
        grad_y = exp_term * cos(x(1)) * sin(x(2)) + exp_term * cos(x(1)) * cos(x(2)) * (2*x(2) - 2*pi);
        g = [grad_x, grad_y];
    end
end
