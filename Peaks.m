...
% s=mesh(X,Y,Z);
fig = figure;
set(fig, 'Units', 'inches', 'Position', [0, 0, 8, 8]); % Width=6in, Height=4in
s=surf(X,Y,Z,'EdgeColor','none','FaceAlpha',0.95);
% s.FaceColor = 'flat';
xlabel('x')
ylabel('y')
zlabel('f')
...
z=[exp((- (x + 1)^2 - y^2))*(2*x + 2)/3 + 3*exp((- (y + 1)^2 - x^2))*(2*x - 2) + exp((- x^2 - y^2))*(30*x^2 - 2) - 6*x*exp((- (y + 1)^2 - x^2))*(x - 1)^2 - 2*x*exp((- x^2 - y^2))*(10*x^3 - 2*x + 10*y^5),...
...
f=3*(1-x(1))^2*exp(-x(1)^2-(x(2)+1)^2)-10*(x(1)/5-x(1)^3-x(2)^5)*exp(-x(1)^2-x(2)^2)-exp(-(x(1)+1)^2-x(2)^2)/3;

if nargout > 1 % gradient required
    g=[exp((- (x(1) + 1)^2 - x(2)^2))*(2*x(1) + 2)/3 + 3*exp((- (x(2) + 1)^2 - x(1)^2))*(2*x(1) - 2) + exp((- x(1)^2 - x(2)^2))*(30*x(1)^2 - 2) - 6*x(1)*exp((- (x(2) + 1)^2 - x(1)^2))*(x(1) - 1)^2 - 2*x(1)*exp((- x(1)^2 - x(2)^2))*(10*x(1)^3 - 2*x(1) + 10*x(2)^5),...
