...
s=surf(X,Y,Z,'EdgeColor','none','FaceAlpha',0.5);
% s.FaceColor = 'flat';
xlabel('x')
ylabel('y')
zlabel('f')
...
fig = figure;
set(fig, 'Units', 'inches', 'Position', [0, 0, 16, 8]); % Width=6in, Height=4in
...
plot(optimhistory.fval,'k',"LineWidth",3)
xlabel('Iteration')
ylabel('f(x)')
lgd =legend('GD','PGD','SA','SPGD','Fmincon');
set(lgd, 'FontSize', 10);
xlim([0, length(history_sa.fval)])
disp("Time:")
T_g*1000
T_pg*1000
T_fmincon*1000
...
z=[(2*x - 2)*(sin(3*pi*y)^2 + 1) + 6*pi*cos(3*pi*x)*sin(3*pi*x),...
   (2*y - 2)*(sin(2*pi*y)^2 + 1) + 6*pi*cos(3*pi*y)*sin(3*pi*y)*(x - 1)^2 + 4*pi*cos(2*pi*y)*sin(2*pi*y)*(y - 1)^2];
end
...
if nargout > 1 % gradient required
    g=[(2*x(1) - 2)*(sin(3*pi*x(2))^2 + 1) + 6*pi*cos(3*pi*x(1))*sin(3*pi*x(1)),...
   (2*x(2) - 2)*(sin(2*pi*x(2))^2 + 1) + 6*pi*cos(3*pi*x(2))*sin(3*pi*x(2))*(x(1) - 1)^2 + 4*pi*cos(2*pi*x(2))*sin(2*pi*x(2))*(x(2) - 1)^2];
end
